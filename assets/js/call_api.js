...
async function fetchApiWithPageNumberManagerAccount(pageNumber) {
    try {
        ...
        const getAccountUrl = `https://localhost:7013/api/LectureSchedule/Registered_Calendar?pageIndex=${pageNumber}&pageSize=${10}&check=${0}&Name=${'string'}`;
...
// Gọi API với số trang được truyền vào
async function fetchApiWithPageNumberManagerRegisted(pageNumber) {
    try {
        var token = localStorage.getItem("login");
        const getAccountUrl = `https://localhost:7013/api/LectureSchedule?token=${token}&pageIndex=${pageNumber}&pageSize=${10}`;
        const response = await fetch(getAccountUrl);
        const data = await response.json();
